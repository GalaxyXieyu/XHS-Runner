...
async function main() {
  ...
  const model = (args.model || 'nanobanana') as 'nanobanana' | 'jimeng' | 'ark';

  if (!idea) {
    console.error('Usage: npx tsx src/server/scripts/batchGenerateSmoke.ts --idea "主题" --count 2 [--execute] [--model nanobanana|jimeng|ark]');
    process.exitCode = 2;
    return;
  }
