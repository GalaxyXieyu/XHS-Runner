...
This repo is an Electron + Next.js app. The main process and services live in `electron/` (IPC, workflow, capture, DB, metrics), while the renderer UI lives in `pages/`. Operational docs live in `docs/`, and local runtime artifacts are stored in `data/` (keep generated data out of commits).
...
- JavaScript uses 2-space indentation, single quotes, and semicolons. Follow patterns in `pages/index.js` and `electron/*.js`.
- Module files are camelCase (for example, `workflowService.js`), and React component names are PascalCase.
