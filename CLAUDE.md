...
**文件大小限制**：
- 单个文件应保持在 **≤ 800 行**（除非有充分理由）
- 组件超过 800 行时应拆分为多个子组件和 hooks
- 避免巨型组件：职责分离、逻辑提取、UI 拆分

**测试要求**（强制）：
1. **API 测试优先**：每个功能必须先通过 API 测试
2. **Chrome UI/E2E 测试**：API 测试通过后使用 chrome-dev mcp UI 验证（确保用户交互没有报错和逻辑不会太过冗余复杂）
3. **UX 流程验证**：
   - 运行 `npm run dev:next`
