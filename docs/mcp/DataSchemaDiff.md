# 数据结构差异排查（xhs-core vs 现有 schema）

## 重点差异概览
- xhs-core 的笔记相关工具强依赖 `note_id` + `xsec_token`，现有 topics 中 `xsec_token` 可选但部分流程未贯通。
