...
import { resolveUserDataPath } from '../../src/server/runtime/userDataPath';
import { BrowserManager } from '../../src/server/services/xhs/core/browser/browser.manager';
...
async function downloadWithFetch(url: string): Promise<{ buffer: Buffer; contentType: string }> {
...
async function downloadWithBrowser(url: string): Promise<{ buffer: Buffer; contentType: string }> {
  ...
  try {
    ...
    const response = await page.goto(url, { waitUntil: 'networkidle2', timeout: DOWNLOAD_TIMEOUT_MS });
    if (!response) {
...
async function ensureCached(originalUrl: string, urlForFetch: string): Promise<{ dataPath: string; contentType: string }> {
  ...
  const promise = (async () => {
    ...
    try {
      const { buffer, contentType } = await downloadWithFetch(urlForFetch);
      ...
      const { buffer, contentType } = await downloadWithBrowser(urlForFetch);
      const tmpPath = `${dataPath}.tmp`;
      fs.writeFileSync(tmpPath, buffer);
      fs.renameSync(tmpPath, dataPath);
      writeMeta(metaPath, { contentType, originalUrl });
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    for (const candidate of candidates) {
      try {
        const { dataPath, contentType } = await ensureCached(originalUrl, candidate);
