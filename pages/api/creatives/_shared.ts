...
let cachedCreativeService: any = null;

export async function getCreativeService() {
  await ensureInit();
  if (cachedCreativeService) return cachedCreativeService;
  const mod = await import('../../../src/server/services/xhs/creativeService');
  cachedCreativeService = mod;
  return cachedCreativeService;
}
